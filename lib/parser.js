...
import * as file from './fileformat.cjs';
import * as osm from './osmformat.cjs';
...
export class BlobParser extends Transform {
    ...
    _transform(buf, enc, next) {
        ...
        if (this._mode === SIZE) {
            ...
            this._header = file.BlobHeader.decode(buf.subarray(0, this._waiting));
            this._mode = BLOB;
            ...
            this._blob = file.Blob.decode(buf.subarray(0, this._waiting));
            let h = this._header;
            this._mode = SIZE;
            let nbuf = buf.subarray(this._waiting);
            this._offset += this._waiting;
...
export class PrimitivesParser extends Transform {
    constructor({ withInfo = true, withTags = true }) {
        super({ objectMode: true, highWaterMark: 1 });
    ...
    _transform(chunk, enc, callback) {
        if (chunk.type === 'OSMHeader') {
            this._osmheader = osm.HeaderBlock.decode(chunk.data);
        } else if (chunk.type === 'OSMData') {
            const block = osm.PrimitiveBlock.decode(chunk.data);
            const opts = {
            ...
            // Output:
            const items = [];
            for (let group of block.primitivegroup) {
                if (group.dense)
...
function parseDenseNodes(dense, opts, results) {
    ...
    for (; offset < dense.id.length; offset++) {
        ...
        const tbl = opts.stringtable;
        const tags = {};
        for (; tagsOffset < dkv.length - 1 && dkv[tagsOffset] !== 0; tagsOffset += 2) {
            let k = tbl[dkv[tagsOffset]];
            let v = tbl[dkv[tagsOffset + 1]];
            tags[k] = v;
        }
        // Skip the 0
        tagsOffset += 1;
...
function parseWay(data, opts, results) {
    const tags = {};
    if (data.keys.length != data.vals.length)
        throw new Error('parseWay: keys.length != vals.length');
    ...
    let ref = 0;
    let refs = data.refs.map(r => ref += r);
...
function parseRelation(data, opts, results) {
    ...
    let id = 0;
    const members = [];
    if (data.roles_sid.length != data.memids.length || data.memids.length != data.types.length)
        throw new Error('parseRelation: members arrays not same length');
    for (let i = 0; i < data.memids.length; i++) {
        id += data.memids[i];
        let typeStr;
        switch (data.types[i]) {
            case 0:
...
export class BlobEncoder extends Transform {
    ...
    _transform(blob, enc, next) {
        var blobMessage = file.Blob.encode({
            zlib_data: blob.zlib_data
        });
        var blobHeader = file.BlobHeader.encode({
            type: blob.type,
